...
----------------------------------------------------------------------------*/
#pragma once
#ifndef __REGIONSUPPORT__
#define __REGIONSUPPORT__
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <float.h>
	...
	image_int * region)
{
	...
	xsize = angles->xsize;
	ysize = angles->ysize;
	logNT = 3.0 * (log10((double)xsize) + log10((double)ysize)) / 2.0;
...
ntuple_list lsd(image_double image)
{
	...
	double sigma_scale = 0.7; /* Sigma for Gaussian filter is computed as
							  sigma = sigma_scale/scale.                    */
	double quant = 1.0;       /* Bound to the quantization error on the
							  gradient norm. default 2.0                                */
	double ang_th = 22.5;     /* Gradient angle tolerance in degrees.           */
	double eps = -10000.0;         /* Detection threshold, -log10(NFA).              */
	double density_th = 0.6;  /* Minimal density of region points in rectangle. */
